...
parser.add_argument("-s","--select_split",type = int, default=1,help='1,2,3,4,5')
parser.add_argument("-k","--k_neg_pair",type = int, default=2)

parser.add_argument("--alp",type = float, default=0.6)
parser.add_argument("--margin",type = float, default=0.4)
# Hyper Parameters

parser.add_argument("-t","--dataset",type = str, default='KINFACE2',help='TSKINFACE or KINFACE1 or KINFACE2')
...
def main():
    ...
    if args.dataset == 'KINFACE1':
        ...
        train_loader,test_index_for_son,test_index_for_dau,MAKE_test_index_for_fs,MAKE_test_index_for_ms,MAKE_test_index_for_fd,MAKE_test_index_for_md = get_train_loader(args.dataset,SELECT_SPLIT,split_set,BATCH_SIZE,args.picroot)
    ...
    for episode in range(EPISODE):
        ...
        if (episode) % args.eval_each_episode == 0:
            if not args.is_save_log:
                ...
                print("Parameters a:{:.1f},m:{:.1f},loss:{},conv:{},dataset:{},split:{:d},k_neg_pair:{:d},learning_rate:{:f}".format(alp,margin,args.loss,args.conv,args.dataset,SELECT_SPLIT,K_PAIR,LEARNING_RATE))
        ...
        if episode % args.eval_each_episode == 0:
                ...
                if Mean > last_Mean:
                    ...
                    if args.is_save_weights:
                            ...
                            torch.save(feature_encoder.state_dict(),
                                join(args.modelpath,"feature_encoder_"+args.dataset+"_"+args.addtion +"_K_PAIR_"+str(K_PAIR)+"_SPLIT_"+ str(SELECT_SPLIT) +"_a_"+str(alp)+"_m_"+str(margin)+ ".pkl"))
                            torch.save(relation_network.state_dict(),
                                join(args.modelpath, "relation_network_"+args.dataset+"_"+args.addtion +"_K_PAIR_"+str(K_PAIR)+"_SPLIT_"+ str(SELECT_SPLIT) +"_a_"+str(alp)+"_m_"+str(margin)+ ".pkl"))
            ...
            if args.is_save_log:
                ...
                logger.info("Parameters a:{:.1f},m:{:.1f},loss:{},conv:{},dataset:{},split:{:d},k_neg_pair:{:d},learning_rate:{:f}".format(alp,margin,args.loss,args.conv,args.dataset,SELECT_SPLIT,K_PAIR,LEARNING_RATE))
                if args.dataset == 'txt':
                    print('Mean:', Mean)
